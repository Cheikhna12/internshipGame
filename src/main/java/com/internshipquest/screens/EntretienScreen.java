...
public class EntretienScreen implements Screen {
    ...
    private void nextQuestion(InternshipQuestGame game) {
        if (entretien.verifFinEntretien(game)) {
            ...
            if (entretien.isAccepted()) {
                resultMessage = "CONGRATULATIONS! You are hired!\n\nSalary: +" + entreprise.getSalaire() + " euros";
                try {
                    SoundManager.playSound("hired", 1.0f);
                } catch (Exception e) {
                    System.out.println("[ENTRETIEN] Sound error hired: " + e.getMessage());
                }
                ...
                resultMessage = "Sorry, your profile does not match our expectations...\n\nStress: +10";

                try {
                    SoundManager.playSound("rejected", 1.0f);
                } catch (Exception e) {
                    System.out.println("[ENTRETIEN] Sound error rejected: " + e.getMessage());
                }

                hero.setStress(Math.min(100, hero.getStress() + 10));
                System.out.println("[ENTRETIEN] Rejected. Stress: +10");
            }
    ...
    public void render(float delta) {
        ...
        game.font.draw(game.batch, "Interview at: " + entreprise.getName(), 50, 900);
    ...
    private void renderResultState(float delta) {
        ...
        game.font.draw(game.batch, "Interview score: " + entretien.getScoreEntretien() + " / " + entretien.getTotalQuestions(), 50, 480);
        game.font.draw(game.batch, "Number of questions: " + entretien.getTotalQuestions(), 50, 450);
    ...
    private void handleInput() {
        ...
        if (choiceIndex != -1) {
            System.out.println("[ENTRETIEN] Answer chosen: " + (choiceIndex == 0 ? "TECH" : "SOFT"));
            ...
            try {
                ...
                System.out.println("[ENTRETIEN] Sound error feedback: " + e.getMessage());
            }
